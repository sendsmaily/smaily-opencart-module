...
class ControllerModuleSmailyForOpencart extends Controller {
    ...
    public function index() {
        ...
        $data['customer_cron_url'] = $this->config->get('config_url') . 'index.php?route=smailyforopencart/cron_customers&token=[token]';
        ...
        $data['cart_cron_url'] = $this->config->get('config_url') . 'index.php?route=smailyforopencart/cron_cart&token=[token]';
    ...
    public function ajaxGetAutoresponders() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ( (int) $this->model_setting_setting->getSettingValue('smaily_for_opencart_validated') === 1) {
                $autoresponders = $this->model_smailyforopencart_admin->apiCall('autoresponder');
