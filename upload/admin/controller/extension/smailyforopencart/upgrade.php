...
class ControllerExtensionSmailyForOpencartUpgrade extends Controller {
     ...
     * Event handler for admin/model/setting/modification/addModification/after
     *
     * @return void
     */
    public function upgrade($route, $args) {
        // Version 2.3 $args structure changed
        if (is_array($args[0])) {
            $args = $args[0];
        }
        ...
        if (! array_key_exists('code', $args) ||
            $args['code'] !== 'smaily_for_opencart_extension'
        ) {
            return;
        }
        ...
        $smaily_module = $this->model_setting_modification->getModificationByCode('smaily_for_opencart_extension');
