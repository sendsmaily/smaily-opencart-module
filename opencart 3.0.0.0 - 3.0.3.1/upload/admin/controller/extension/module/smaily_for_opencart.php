...
class ControllerExtensionModuleSmailyForOpencart extends Controller {
    ...
    public function index() {
        ...
        // Api key.
        $data['api_key_title'] = $this->language->get('api_key_title');
        $data['api_key_placeholder'] = $this->language->get('placeholder_api_key');
        $data['validated'] = $this->model_setting_setting->getSettingValue('module_smaily_for_opencart_validated');
        // Validate button.
        ...
        // Small texts.
        $data['small_api_key'] = $this->language->get('small_api_key');
        ...
        $data['small_autoresponder'] = $this->language->get('small_autoresponder');
        ...
        // Subscriber autoresponder error.
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/smaily_for_opencart', 'user_token=' . $this->session->data['user_token'], true)
        );

        // Save and cancel button href-s.
        $data['action'] = $this->url->link('extension/module/smaily_for_opencart', 'user_token=' . $this->session->data['user_token'], true);
        ...
        // Api key.
        if (isset($this->request->post['module_smaily_for_opencart_api_key'])) {
            $data['api_key'] = $this->request->post['module_smaily_for_opencart_api_key'];
        } else {
            $data['api_key'] = $this->config->get('module_smaily_for_opencart_api_key');
        }
        ...
        // Customer sync autoresponder.
        if (isset($this->request->post['module_smaily_for_opencart_autoresponder'])) {
            $data['autoresponder'] = json_decode(html_entity_decode($this->request->post['module_smaily_for_opencart_autoresponder']), true);
        } else {
            $data['autoresponder'] = json_decode(html_entity_decode($this->config->get('module_smaily_for_opencart_autoresponder')), true);
        }
        ...
        // Customer sync token.
        if (isset($this->request->post['module_smaily_for_opencart_sync_token'])) {
        ...
        if (isset($this->request->post['module_smaily_for_opencart_cart_token'])) {
    ...
    protected function validate() {
        ...
        // Validate autoresponder selected for subscriber sync.
        if (isset($this->request->post['module_smaily_for_opencart_enable_subscribe'])
            && (int) $this->request->post['module_smaily_for_opencart_enable_subscribe'] === 1
            ) {
                if (empty($this->request->post['module_smaily_for_opencart_autoresponder'])){
                    $this->error['autoresponder'] = $this->language->get('autoresponder_error');
                }
        }
        ...
        if (isset($this->request->post['module_smaily_for_opencart_enable_abandoned'])
            ...
            // Delay time.
            if ($this->request->post['module_smaily_for_opencart_cart_delay'] < 30) {
                $this->error['cart_delay'] = $this->language->get('cart_delay_error');
            }
        }
        // Validate credentials when saving settings.
        // Previously saved settings.
        $prev_settings = $this->model_setting_setting->getSetting('module_smaily_for_opencart');
        // Credentials.
        $smaily_subdomain = $this->db->escape($this->request->post['module_smaily_for_opencart_subdomain']);
        $smaily_username =  $this->db->escape($this->request->post['module_smaily_for_opencart_username']);
        $smaily_password = $this->db->escape($this->request->post['module_smaily_for_opencart_password']);
        // Check if credentials validation passes.
        $credentials_status = $this->validateSmailyCredentials($smaily_subdomain, $smaily_username, $smaily_password);
        if (! array_key_exists('success', $credentials_status)) {
            // Error message.
            $this->error['validate'] = $this->language->get('error_validate');
            // Write validation status to 0.
            $prev_settings['module_smaily_for_opencart_validated'] = 0;
            $this->model_setting_setting->editSetting('module_smaily_for_opencart', $prev_settings);
        }
    ...
    public function validateSmailyCredentials( string $subdomain, string $username, string $password ) {
        ...
        // cUrl
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://' . $subdomain . '.sendsmaily.net/api/autoresponder.php');
    ...
    public function ajaxGetAutoresponders() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ( (int) $this->model_setting_setting->getSettingValue('module_smaily_for_opencart_validated') === 1) {
                $autoresponders = $this->model_extension_smailyforopencart_admin->apiCall('autoresponder');
                // Return autoresponders
                if (!empty($autoresponders)) {
                    foreach ($autoresponders as $autoresponder) {
                        if (!empty($autoresponder['id']) && !empty($autoresponder['name'])) {
                            $list[$autoresponder['id']] = trim($autoresponder['name']);
                        }
                    }
                }
            }
    ...
    public function install() {
        ...
        $this->model_setting_event->addEvent('smaily_order', 'catalog/controller/checkout/confirm/after', 'extension/smailyforopencart/order/removeSent');
    }
